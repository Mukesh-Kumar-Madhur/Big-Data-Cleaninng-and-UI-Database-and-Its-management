...
def create_kroger_df():
    # Specify the path of Excel file
    excel_file = 'Data\Master Forecast File.xlsx'

    kroger_df = pd.read_excel(excel_file, sheet_name='Kroger', skiprows=160)
    pd.set_option('display.max_columns', None)

    ## Extracting date records in a df
    date_df = pd.read_excel(excel_file, sheet_name='Kroger', skiprows=150)
    ...
    kroger_df = kroger_df.dropna(axis=1, how='all')
    ...
    kroger_df = kroger_df.dropna(how='all')

    # Remove records where the value in the "unit" column is empty or NaN
    kroger_df = kroger_df[kroger_df['Units'].notna() & (kroger_df['Units'] != '')]

    # Convert the "data" column to integers
    kroger_df['Date'] = kroger_df['Date'].astype(int)


    # Get unique units from kroger_df
    unique_units = kroger_df['Units'].unique()
    ...
    for unit in unique_units:
        ...
        brand = kroger_df[kroger_df['Units'] == unit]['Brand'].iloc[0]
        packs_per_case = kroger_df[kroger_df['Units'] == unit]['Packs Per Case'].iloc[0]
        
        # Filter kroger_df for the current unit
        unit_data = kroger_df[kroger_df['Units'] == unit]
    ...
    # template_df['Sales U'] = filled_df['Data_weekly']   # Planing ot use Sales in unit for the data part.
    template_df['Retailers'] = "Kroger"
    ...
    for index, row in template_df.iterrows():
        ...
        # Extract the number within parentheses in the week string from template_df
        week_num_template = re.search(r'\((.*?)\)', week).group(1)
        
        for column in date_df.columns:
            # Extract the number within parentheses in the column name from date_df
            week_num_date = re.search(r'\((.*?)\)', column).group(1)
            
            if week_num_template == week_num_date:
                for index, date in date_df[column].items():
                    year = date.year
                    if year == year_temp:
                        specific_record = date_df.loc[index, column]
...
### Saving CSV file to local storage
    # name = 'Kroger_Template'
