...
def create_walmart_df():
    
    # Specify the path of Excel file
    excel_file = 'Data\Master Forecast File.xlsx'
    walmart_df = pd.read_excel(excel_file, sheet_name='Walmart', skiprows=232)

    ## Extracting date records in a df
    date_df = pd.read_excel(excel_file, sheet_name='Walmart', skiprows=222)

    date_df = date_df.head(10)
    ...
    walmart_df = walmart_df.dropna(axis=1, how='all')
    ...
    walmart_df = walmart_df.dropna(how='all')


    # Remove records where the value in the "unit" column is empty or NaN
    walmart_df = walmart_df[walmart_df['Units'].notna() & (walmart_df['Units'] != '')]

    # Convert the "data" column to integers
    walmart_df['Date'] = walmart_df['Date'].astype(int)

    # Get unique units from walmart_df
    unique_units = walmart_df['Units'].unique()
    ...
    for unit in unique_units:
        ...
        brand = walmart_df[walmart_df['Units'] == unit]['Brand'].iloc[0]
        packs_per_case = walmart_df[walmart_df['Units'] == unit]['Packs Per Case'].iloc[0]
        
        # Filter walmart_df for the current unit
        unit_data = walmart_df[walmart_df['Units'] == unit]
    ...
    template_walmart = pd.DataFrame(columns=columns)


    # Fill the "Item Description" column of template_df from the "Units" column of walmart_df

    template_walmart['Item Description'] = filled_df['Units']
    template_walmart['Brand'] = filled_df['Brand']
    template_walmart['Packs Per Case'] = filled_df['Packs Per Case']
    template_walmart['Year'] = filled_df['Year']
    template_walmart['Week'] = filled_df['Week']
    template_walmart['Sales U'] = filled_df['Data_weekly']
    template_walmart['Retailers'] = "Walmart"


    date_L = []

    for index, row in template_walmart.iterrows():
        week = row['Week']
        year_temp = row['Year']
    ...
    # Filling Date column     
    template_walmart["Date"] = date_L

    df_sales = pd.read_excel(excel_file, sheet_name='Walmart', skiprows=2)
    ...
    template_walmart['Date'] = pd.to_datetime(template_walmart['Date'])
    df_pivot['Date'] = pd.to_datetime(df_pivot['Date'])

    # Merge the dataframes on 'Date' and 'Item Description'
    merged_df = pd.merge(template_walmart, df_pivot, on=['Date', 'Item Description'], how='left')


    template_walmart['Sales $'] = merged_df['$ Sales']
    template_walmart['Sales U'] = merged_df['Unit Sales']
    template_walmart['Avg Sell Price'] = merged_df['Avg $/Unit']
    # template_walmart['Retail'] = merged_df['']
    template_walmart['Store Count'] = merged_df['Stores']
    template_walmart['In Stock'] = merged_df['In Stock_y']
    template_walmart['PPSPW'] = merged_df['PPSPW_y']

### Creating CSV file to local storage.
    # name = 'Walmart_Template'
    # template_walmart.to_csv(f'{name}.csv', index=False)
    # print(f"{name} CSV Dataset Created Successfully")
    
    
    return template_walmart


if __name__ == "__main__":
    df = create_walmart_df()
    print(df)