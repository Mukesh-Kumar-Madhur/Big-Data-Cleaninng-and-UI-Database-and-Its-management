...
def create_wakefern_df():
    # Specify the path of Excel file
    excel_file = 'Data\Master Forecast File.xlsx'

    template_df = pd.read_excel(excel_file, sheet_name='WakefernShopRite', skiprows=286)
    # pd.set_option('display.max_columns', None)

    template_df = template_df.head(430-286)
    ## Extracting date records in a df
    date_df = pd.read_excel(excel_file, sheet_name='WakefernShopRite', skiprows=274)
    # date_df
    date_df = date_df.head(12)
    ...
    date_df = date_df.apply(pd.to_datetime) # Convert all columns to USA date format

    template_df = template_df.dropna(how='all') # Drop all records with NaN values

    # Remove records where the value in the "unit" column is empty or NaN
    template_df = template_df[template_df['Units'].notna() & (template_df['Units'] != '')]

    # Convert the "data" column to integers
    template_df['Date'].dtype
    template_df['Date'] = template_df['Date'].astype(int)


    # Get unique units from amazon_df
    unique_units = template_df['Units'].unique()


    data_to_fill = []   # Initialize an empty list to store the data
    ...
    for unit in unique_units:
        ...
        brand = template_df[template_df['Units'] == unit]['Brand'].iloc[0]
        packs_per_case = template_df[template_df['Units'] == unit]['Packs Per Case'].iloc[0]
            
        # Filter template_df for the current unit
        unit_data = template_df[template_df['Units'] == unit]
    ...
    template_df['Retailers'] = "WakeFern"      ## This is the name of the Retailers, Please rename here if not getting correct
    ...
    df_sales = pd.read_excel('Data\Master Forecast File.xlsx', sheet_name='WakefernShopRite', skiprows=2)

        # Keeping only required records
    df_sales = df_sales.head(272)
    ...
    df_pivot = df_pivot.dropna(how='all', subset=df_pivot.columns.difference(['Item Description']))
