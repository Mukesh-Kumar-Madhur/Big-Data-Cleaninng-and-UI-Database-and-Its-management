...
def create_publix_df():

    # Specify the path of Excel file
    excel_file = 'Data\Master Forecast File.xlsx'

    publix_df = pd.read_excel(excel_file, sheet_name='Publix', skiprows=313).head(463-313)
    # pd.set_option('display.max_columns', None)

    ## Extracting date records in a df
    date_df = pd.read_excel(excel_file, sheet_name='Publix', skiprows=300)
    date_df = date_df.head(13)  # date_df
    ...
    for unit in unique_units:
        ...
        brand = publix_df[publix_df['Units'] == unit]['Brand'].iloc[0]
        packs_per_case = publix_df[publix_df['Units'] == unit]['Packs Per Case'].iloc[0]
        
        # Filter publix_df for the current unit
        unit_data = publix_df[publix_df['Units'] == unit]
    ...
    template_df['Retailers'] = "Publix"      ## This is the name of the Retailers, Please rename here if not getting correct
    ...
    df_sales = pd.read_excel('Data\Master Forecast File.xlsx', sheet_name='Publix', skiprows=2)

    # Keeping only required records
    df_sales = df_sales.head(187)
